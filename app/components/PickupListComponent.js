...
class PickupListComponent extends React.Component{
	...
	async detailScreen(mode, orderId) {
		...
		this.props.navigation.navigate('ProductDetailScreen', { mode: mode, returnId: orderId });
					...
					<ScrollView style={{flex: 1, paddingHorizontal: 15}} keyboardShouldPersistTaps={'always'}>
...

						{
							...
							this.props.orderData.map((data, index) => (
								...
								<TouchableOpacity key={index} style={{...style.cardWrapper, backgroundColor: '#FFFFFF'}} onPress = {() => this.detailScreen(this.props.index, data.salesreturnRequestId)}>
									<View style={{...style.cardHeaderWrapper}}>

										<View style={{flex: 1, flexDirection: 'row', marginBottom: 2}}>
											<View style={{flex: 1, justifyContent: 'center'}}>
												<Text style={{fontFamily: 'Gilroy-Bold', fontSize: 14}}>{data.oCustomerName}</Text>
											</View>
											<View style={{flex: 1, alignItems: 'center', justifyContent: 'center'}}>
												<Text style={{fontFamily: 'Gilroy-Bold', fontSize: 12, flexWrap: 'wrap'}}>Return No: {(data.salesreturnRequestNo != null) ? data.salesreturnRequestNo : null}</Text>
											</View>
											<View style={{flex: 1, alignItems: 'flex-end', justifyContent: 'center'}}>
												<Text style={{fontFamily: 'Gilroy-Bold', fontSize: 12, flexWrap: 'wrap'}}>
													{(data.collectionDate != null) ? Moment(data.collectionDate).format('DD-MMM-YYYY') : null}
												</Text>
											</View>
										</View>
										...
										<View style={{flex: 1, flexDirection: 'row', marginBottom: 1}}>
											<View style={{flex: 2, justifyContent: 'center'}}>
												<Text style={{fontFamily: 'Gilroy-Medium', fontSize: 14}}>{data.customerHouseNo}</Text>
												<Text style={{fontFamily: 'Gilroy-Medium', fontSize: 14}}>{data.customerArea}</Text>
												<Text style={{fontFamily: 'Gilroy-Medium', fontSize: 14}}>{data.customerCity} - {data.customerPincode}</Text>
												<Text style={{fontFamily: 'Gilroy-Medium', fontSize: 14}}>{data.customerCountry}</Text>
											</View>
											...
											<View style={{flex: 1, alignItems: 'center', justifyContent: 'center', flexDirection: 'row'}}>
												<View style={{flex: 0.5, marginRight: 10 }}>
													
													{
														...
														(data.customerMobileNo1 != null && data.customerMobileNo2 != null ) ?
...
export default withNavigation(PickupListComponent)